...
import { downstreamRequestsCodexResponsesLite } from '../../responses-lite.ts';
...
const withDefaultInstructions = <T extends { instructions?: string | null }>(body: T): T =>
...
export const injectDefaultInstructions = async <TResult>(
  ...
  if (!downstreamRequestsCodexResponsesLite(ctx.headers, ctx.payload)) {
