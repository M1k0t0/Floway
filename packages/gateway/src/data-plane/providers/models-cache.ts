...
import { defaultsForProvider, resolveEffectiveFlags, type Fetcher, type Provider, type ProviderModel } from '@floway-dev/provider';
...
const modelsWithCurrentCodexFlags = async (
  ...
  if (instance.kind !== 'codex') return models;
  const record = await getRepo().upstreams.getById(instance.upstream);
  if (record?.kind !== 'codex') return models;
