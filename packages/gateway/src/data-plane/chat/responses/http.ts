...
import type { CanonicalResponsesPayload, ResponsesCompactPayload, ResponsesPayload } from '@floway-dev/protocols/responses';
...
const parsePayload = (requestBody: RequestBody): CanonicalResponsesPayload =>
  canonicalizeResponsesPayload(JSON.parse(new TextDecoder().decode(requestBody.bytes)) as ResponsesPayload | ResponsesCompactPayload);
