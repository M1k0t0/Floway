...
export const wrapResponsesOutputForStorage = async function* (
  ...
  args: {
    ...
    readonly beforeCommitSnapshot?: (responseId: string) => void | Promise<void>;
  },
): AsyncGenerator<ProtocolFrame<ResponsesStreamEvent>> {
  const { store, upstream, snapshotMode, targetApi, responseId, beforeCommitSnapshot } = args;
  ...
  for await (const frame of frames) {
    ...
    if (event.type === 'response.completed' || event.type === 'response.incomplete') {
      ...
      if (snapshotMode !== 'none') {
        try {
          await beforeCommitSnapshot?.(responseId);
