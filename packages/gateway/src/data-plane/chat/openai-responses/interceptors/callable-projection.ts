...
export interface CallableProjection {
...
export const projectCallables = (request: CanonicalOpenAIResponsesPayload): {
  payload: CanonicalOpenAIResponsesPayload;
  names: CallableProjection;
} => {
...
export const restoreCallableEvents = async function* (
...
// Project each private translated dispatch inside the server-tool loop. Native
// Responses and the loop's canonical request never receive allocated wire names.
export const dispatchWithCallableProjection = async (
  invocation: OpenAIResponsesInvocation,
  dispatch: (invocation: OpenAIResponsesInvocation) => Promise<ExecuteResult<ProtocolFrame<OpenAIResponsesStreamEvent>>>,
): Promise<ExecuteResult<ProtocolFrame<OpenAIResponsesStreamEvent>>> => {
  if (invocation.targetApi === 'openaiResponses') return await dispatch(invocation);
  const { payload, names } = projectCallables(invocation.payload);
  // Translators can share nested schemas with their source. Keep target rules
  // and provider mutations separate from canonical history and response echoes.
  const result = await dispatch({ ...invocation, payload: klona(payload) });
  return result.type === 'events' ? { ...result, events: restoreCallableEvents(result.events, names) } : result;
};
